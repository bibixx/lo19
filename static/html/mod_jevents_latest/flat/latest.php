...
class FlatModLatestView extends DefaultModLatestView
{
	...
	function displayLatestEvents(){
		...
		if(isset($this->eventsByRelDay) && count($this->eventsByRelDay)){
			...
			$content .= $this->modparams->get("modlatest_templatetop") ? $this->modparams->get("modlatest_templatetop") : '<section><h2>Terminarz</h2><main class="timetable">';
				...
				foreach($daysEvents as $dayEvent){
					...
					foreach($this->splitCustomFormat as $condtoken) {
						...
						foreach($condtoken['data'] as $token) {
							...
							if( ($dateParm != "%F" && $dateParm != "%d %B %Y") || $timeNo < 1 ){
			...
			$content .=$this->modparams->get("modlatest_templatebottom") ? $this->modparams->get("modlatest_templatebottom") : '</main><aside>Harmonogram roku dostępny jest <a href="'.JURI::root().'organizacja-roku-szkolnego">tutaj</a></aside></section>'."\n";
		} else if ($this->modparams->get("modlatest_NoEvents", 1)){
			$content .= $this->modparams->get("modlatest_templatetop") ? $this->modparams->get("modlatest_templatetop") : '<section><h2>Terminarz</h2><main class="timetable">';
			$templaterow = $this->modparams->get("modlatest_templaterow") ? $this->modparams->get("modlatest_templaterow")  : '<section><strong>%s</strong></section>' . "\n";
